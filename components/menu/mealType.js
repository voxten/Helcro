...
export default function MealType({ onClose, onSubmit }) {
  ...
  const handleMealClick = (meal) => {
    if (selectedMeal === meal) {
      ...
      if (meal !== 'Other') {
        setMealTimeVisible(true); // Show time picker for other meals
      } else {
        setMealTimeVisible(true); // Allow time picker for "Other" as well
      }
    }
  };
      ...
      <View style={mainStyles.modalContainer}>
        ...
        <View style={styles.buttons}>
          ...
          <TouchableOpacity
            ...
            onPress={onSubmit}
            disabled={isNextButtonDisabled} // Disable button if conditions are not met
          >
